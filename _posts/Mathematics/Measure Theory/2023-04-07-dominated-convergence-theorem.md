...
$$\mu\left( \{\lvert u \rvert \geq c\} \cap E \right) \leq \frac{1}{c} \int_E \lvert u \rvert \,d{\mu}$$

이다.

**증명.** $\displaystyle\int_E \lvert u \rvert \,d{\mu} \geq \int_{E\cap \{\lvert u \rvert\geq c\}} \lvert u \rvert \,d{\mu} \geq \int_{E\cap \{\lvert u \rvert\geq c\}} c \,d{\mu} = c \mu\left( \{\lvert u \rvert \geq c\} \cap E \right)$.
...
(2 $\implies$ 1) $\displaystyle\int_E \lvert u \rvert \,d{\mu} = \int_{E \cap \{\lvert u \rvert > 0\}} \lvert u \rvert \,d{\mu} + \int_{E \cap \{\lvert u \rvert = 0\}} \lvert u \rvert \,d{\mu} = 0 + 0 = 0$.

(1 $\implies$ 3) Markov's inequality를 사용하면

$$\mu\left( \left\{\lvert u \rvert \geq \frac{1}{n}\right\} \cap E \right) \leq n\int_E \lvert u \rvert \,d{\mu} = 0$$

이다. 이제 $n\rightarrow\infty$ 일 때 continuity of measure를 사용하면 $\mu\left( \{\lvert u \rvert > 0\} \cap E \right) = 0$ 이다.
...
**증명.** $\mu\left( \{\lvert u \rvert \geq 1\}\cap E \right) \leq \displaystyle\int_E \lvert u \rvert \,d{\mu} < \infty$.[^2] 그러므로

$$\begin{aligned}
        \mu\left( \{\lvert u \rvert = \infty\} \cap E \right) & = \mu\left( \bigcap_{n=1}^\infty \{x \in E : \lvert u(x) \rvert \geq n\} \right) \\
                                               & = \lim_{n \rightarrow\infty} \mu\left( \{\lvert u \rvert \geq n\} \cap E \right) \leq \limsup_{n\rightarrow\infty} \frac{1}{n} \int_E \lvert u \rvert \,d{\mu} = 0
    \end{aligned}$$
...
**따름정리.** $u \in \mathcal{L}^{1}(E, \mu)$ 이면 $\displaystyle\int_E u \,d{\mu} = \int_{E \cap \{\lvert u \rvert < \infty\}} u \,d{\mu}$ 이다.
...
$$[f] = \{g \in \mathcal{L}^{1}(E, \mu) : f \sim g\}.$$
...
**정리.** (지배 수렴 정리) Measurable set $E$와 measurable function $f$에 대하여, $\{f_n\}$이 measurable function의 함수열이라 하자. $E$의 거의 모든 점 위에서 극한 $f(x) = \displaystyle\lim_{n \rightarrow\infty} f_n(x)$ 가 $\overline{\mathbb{R}}$에 존재하고 (점별 수렴) $\lvert f_n \rvert \leq g \quad \mu$-a.e. on $E$ ($\forall n \geq 1$) 를 만족하는 $g \in \mathcal{L}^{1}(E, \mu)$ 가 존재하면,
...
$$A = \left\{\displaystyle x \in E : \lim_{n \rightarrow\infty} f_n(x) \text{가 존재하고}, f_n(x), f(x), g(x) \in \mathbb{R}, \lvert f_n(x) \rvert \leq g(x)\right\}.$$
...
$$\begin{aligned}
        2 \int_E g \,d{\mu} = \int_A 2g \,d{\mu} & = \int_A \liminf_{n \rightarrow\infty} \big(2g - \lvert f_n - f \rvert\big) \,d{\mu} \\
                                               & \leq \liminf_{n \rightarrow\infty} \left( 2 \int_A g \,d{\mu} - \int_A \lvert f_n - f \rvert \,d{\mu} \right) \\
                                               & = 2\int_A g \,d{\mu} - \limsup_{n \rightarrow\infty} \int_A \lvert f_n - f \rvert \,d{\mu} \leq 2 \int_A g \,d{\mu}
    \end{aligned}$$
